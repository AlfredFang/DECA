...
def uneven_light_compensate(gray, block_size):
    average = np.mean(gray)
    rows_new = int(np.ceil(gray.shape[0] / block_size))
    cols_new = int(np.ceil(gray.shape[1] / block_size))
    block_image = np.zeros((rows_new, cols_new), dtype=np.float32)
    for r in range(rows_new):
        for c in range(cols_new):
            rowmin = r * block_size
            rowmax = (r + 1) * block_size
            if rowmax > gray.shape[0]:
                rowmax = gray.shape[0]
            colmin = c * block_size
            colmax = (c + 1) * block_size
            if colmax > gray.shape[1]:
                colmax = gray.shape[1]
            imageROI = gray[rowmin:rowmax, colmin:colmax]
            temaver = np.mean(imageROI)
            block_image[r, c] = temaver
    block_image = block_image - average
    block_image2 = cv2.resize(block_image, (gray.shape[1], gray.shape[0]), interpolation=cv2.INTER_CUBIC)
...
def get_image_hull_mask(image_shape, image_landmarks, ie_polys=None):
    ...
    cv2.fillConvexPoly(hull_mask, cv2.convexHull(np.concatenate((int_lmrks[17:20], int_lmrks[8:9]))), (255, 255, 255))
    cv2.fillConvexPoly(hull_mask, cv2.convexHull(np.concatenate((int_lmrks[24:27], int_lmrks[8:9]))), (255, 255, 255))
    cv2.fillConvexPoly(hull_mask, cv2.convexHull(np.concatenate((int_lmrks[19:25], int_lmrks[8:9]))), (255, 255, 255))
    cv2.fillConvexPoly(hull_mask, cv2.convexHull(np.concatenate((int_lmrks[17:22], int_lmrks[27:28], int_lmrks[31:36], int_lmrks[8:9]))), (255, 255, 255))
    cv2.fillConvexPoly(hull_mask, cv2.convexHull(np.concatenate((int_lmrks[22:27], int_lmrks[27:28], int_lmrks[31:36], int_lmrks[8:9]))), (255, 255, 255))
    # nose
    cv2.fillConvexPoly(hull_mask, cv2.convexHull(int_lmrks[27:36]), (255, 255, 255))
...
def resize_image(image_in, w_s=1000):
    h, w, _ = image_in.shape
    if w < w_s:
        image_out = image_in
    else:
        h_s = int(h * w_s / w)
        image_out = cv2.resize(image_in, dsize=(w_s, h_s))
    return image_out
